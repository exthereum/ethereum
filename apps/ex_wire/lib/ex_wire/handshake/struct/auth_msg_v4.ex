defmodule ExWire.Handshake.Struct.AuthMsgV4 do
  ...
  @type t :: %__MODULE__{
          ...
          remote_public_key: Key.public_key(),
          remote_nonce: binary(),
  ...
  def serialize(auth_msg) do
    [
      auth_msg.signature,
      auth_msg.remote_public_key |> Key.der_to_raw(),
  ...
  def deserialize(rlp) do
    ...
    %__MODULE__{
      signature: signature,
      remote_public_key: remote_public_key |> Key.raw_to_der(),
      remote_nonce: remote_nonce,
      remote_version:
        if(is_binary(remote_version),
  ...
  """
  @spec set_remote_ephemeral_public_key(t, Key.private_key()) :: t
  def set_remote_ephemeral_public_key(auth_msg, my_static_private_key) do
    ...
    {:ok, remote_ephemeral_public_key} =
      Signature.recover(shared_secret_xor_nonce, auth_msg.signature, 0)
